...
public class EnemyMovementAI : MonoBehaviour
{
    ...
    private void MoveEnemy()
    {
        ...
        if (Time.frameCount % Global.targetFrameRateToSpreadPathfindingOver != updateFrameNumber) return;
        ...
        if (currentEnemyPathRebuildCooldown <= 0f || (Vector3.Distance(playerReferencePosition, GameManager.Instance.GetPlayer().GetPlayerPosition()) > Global.playerMoveDistanceToRebuildPath))
        {
            // Reset path rebuild cooldown timer
            currentEnemyPathRebuildCooldown = Global.enemyPathRebuildCooldown;
            ...
            if (movementSteps != null)
            {
                ...
                {
                    // Trigger idle event
                    enemy.idleEvent.OnIdle.Trigger();
    ...
    private IEnumerator MoveEnemyRoutine(Stack<Vector3> movementSteps)
    {
        while (movementSteps.Count > 0)
        {
            ...
            while (Vector3.Distance(nextPosition, transform.position) > 0.2f)
            {
                ...
                enemy.movementToPositionEvent.OnMovementToPosition.Trigger(new MovementToPositionArgs(nextPosition, transform.position, moveSpeed, (nextPosition - transform.position).normalized,false));
