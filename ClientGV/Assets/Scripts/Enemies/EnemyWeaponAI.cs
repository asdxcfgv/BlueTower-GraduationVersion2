...
public class EnemyWeaponAI : MonoBehaviour
{
    ...
    private void FireWeapon()
    {
        ...
        float weaponAngleDegrees = Global.GetAngleFromVector(weaponDirection);

        // Get enemy to player angle
        float enemyAngleDegrees = Global.GetAngleFromVector(playerDirectionVector);

        // Set enemy aim direction
        AimDirection enemyAimDirection = Global.GetAimDirection(enemyAngleDegrees);

        // Trigger weapon aim event
        enemy.aimWeaponEvent.OnWeaponAim.Trigger(new AimWeaponEventArgs(enemyAimDirection, enemyAngleDegrees, weaponAngleDegrees, weaponDirection));
        ...
        if (enemyDetails.enemyWeapon != null)
        {
            ...
            if (playerDirectionVector.magnitude <= enemyAmmoRange)
            {
                ...
                enemy.fireWeaponEvent.OnFireWeapon.Trigger(new FireWeaponEventArgs(true, true, enemyAimDirection, enemyAngleDegrees, weaponAngleDegrees, weaponDirection));
            }
        }
    ...
    private bool IsPlayerInLineOfSight(Vector3 weaponDirection, float enemyAmmoRange)
    {
        ...
        if (raycastHit2D && raycastHit2D.transform.CompareTag(Global.playerTag))
        {
            return true;
        }
