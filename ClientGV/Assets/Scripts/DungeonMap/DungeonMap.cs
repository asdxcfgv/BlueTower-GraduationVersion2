...
public class DungeonMap : SingletonMonobehaviour<DungeonMap>
{
    ...
    private void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0) && GameManager.Instance.GetCurrentGameState() == GameState.dungeonOverviewMap)
        {
            GetRoomClicked();
        }
    }
    ...
    private IEnumerator MovePlayerToRoom(Vector3 worldPosition, Room room)
    {
        ...
        Vector3 spawnPosition = Global.GetSpawnPositionNearestToPlayer(worldPosition);
    ...
    {
        // Set game state
        GameManager.Instance.previousGameState = GameManager.Instance.GetCurrentGameState();
        GameManager.Instance.ChangeState(GameState.dungeonOverviewMap);
